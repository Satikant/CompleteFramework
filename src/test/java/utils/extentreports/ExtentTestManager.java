...
//        test.log(Status.INFO, assertvalue);
        return test;
    }
    public static synchronized ExtentTest reportadd(String testName, String desc, Media assertvalue) {
    	ExtentTest test = extent.createTest(testName, desc);
//    	extentTestMap.put((int) Thread.currentThread().getId(), test);
    	test.log(Status.INFO, assertvalue);
    	return test;
    }
    public static synchronized ExtentTest testinfo(String text, String elementname) {
    	ExtentTest test = extent.createTest(text, elementname);
    	return test;
    }
